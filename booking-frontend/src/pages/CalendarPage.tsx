...
const handleBook = async (slot: Timeslot) => {
  ...
  // A user_id lehet user.user_id vagy user.id – kezeljük mindkettőt
  const userId =
    (user as any).user_id ?? (user as any).id;

  try {
    const booking = await createBooking(userId, slot.timeslot_id);
